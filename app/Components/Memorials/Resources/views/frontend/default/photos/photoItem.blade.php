        ...
        $(document).ready(function(){
           $('.btn-photo-view').click(function(e){
               ...
               $.get(url, function(data){
                   $('body').append(data);
                   $('.loading-animate').fadeOut(400, function(){this.remove()});
                   $('#photo-modal').modal();
               }).fail(function() {
                   $('.loading-animate').fadeOut(400, function(){this.remove()});
               });
            ...
            $(document).on('click', '.btn-comment-reply', function (e) {
                e.preventDefault();
                var thisEl = $(this), parent = thisEl.closest('.comment-item');
                $('.comments-children', parent).toggleClass('hide show');
            });
            $(document).on('click', '.btn-comment-like', function (e) {
                e.preventDefault();
                var thisEl = $(this), action = thisEl.attr('href');
                $.ajaxSetup({
                    headers: {'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')}
                });
                $.post(action).done(function(data) {
                    $('.fa', thisEl).toggleClass('fa-heart-o fa-heart');
                }).fail(function(){
                    alert('Request Timeouts. Please try later!');
                });
            });
            /**
             * Comment Form
            ...
            $(document).on('click', '.btn-comment-submit', function (e) {
                var form = $(this).closest('.form-comment-form'),
                ...
                $.post(action, form.serialize()).done(function(data) {
                    ...
                    if(parent_id == 0) {
                        ...
                        $('#comments-children-'+parent_id).prepend(data);
                    }
                }).fail(function() {
                    $('.loading-animate').fadeOut(400, function(){this.remove()});
                    alert('Request Timeouts. Please try later!');
